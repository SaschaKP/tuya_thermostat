...
#include "tuyanew_fan.h"

namespace esphome {
namespace tuyanew {

static const char *const TAG = "tuyanew.fan";

void TuyaNewFan::setup() {
  if (this->speed_id_.has_value()) {
    this->parent_->register_listener(*this->speed_id_, [this](const TuyaNewDatapoint &datapoint) {
      if (datapoint.type == TuyaNewDatapointType::ENUM) {
  ...
  if (this->oscillation_id_.has_value()) {
    this->parent_->register_listener(*this->oscillation_id_, [this](const TuyaNewDatapoint &datapoint) {
  ...
  if (this->direction_id_.has_value()) {
    this->parent_->register_listener(*this->direction_id_, [this](const TuyaNewDatapoint &datapoint) {
...
void TuyaNewFan::dump_config() {
  LOG_FAN("", "TuyaNew Fan", this);
  if (this->speed_id_.has_value()) {
...
void TuyaNewFan::control(const fan::FanCall &call) {
  ...
  if (this->oscillation_id_.has_value() && call.get_oscillating().has_value()) {
    if (this->oscillation_type_ == TuyaNewDatapointType::ENUM) {
      this->parent_->set_enum_datapoint_value(*this->oscillation_id_, *call.get_oscillating());
    } else if (this->oscillation_type_ == TuyaNewDatapointType::BOOLEAN) {
  ...
  if (this->speed_id_.has_value() && call.get_speed().has_value()) {
    if (this->speed_type_ == TuyaNewDatapointType::ENUM) {
      this->parent_->set_enum_datapoint_value(*this->speed_id_, *call.get_speed() - 1);
    } else if (this->speed_type_ == TuyaNewDatapointType::INTEGER) {
