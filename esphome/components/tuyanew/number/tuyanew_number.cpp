...
#include "tuyanew_number.h"

namespace esphome {
namespace tuyanew {

static const char *const TAG = "tuyanew.number";

void TuyaNewNumber::setup() {
  if (this->restore_value_) {
    this->pref_ = global_preferences->make_entity_preference<float>(this);
  }

  this->parent_->register_listener(this->number_id_, [this](const TuyaNewDatapoint &datapoint) {
    if (datapoint.type == TuyaNewDatapointType::INTEGER) {
...
void TuyaNewNumber::dump_config() {
  LOG_NUMBER("", "TuyaNew Number", this);
