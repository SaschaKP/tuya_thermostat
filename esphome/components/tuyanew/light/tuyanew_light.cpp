...
#include "tuyanew_light.h"
#include "esphome/core/helpers.h"

namespace esphome {
namespace tuyanew {

static const char *const TAG = "tuyanew.light";

void TuyaNewLight::setup() {
  if (this->color_temperature_id_.has_value()) {
    this->parent_->register_listener(*this->color_temperature_id_, [this](const TuyaNewDatapoint &datapoint) {
  ...
  if (this->dimmer_id_.has_value()) {
    this->parent_->register_listener(*this->dimmer_id_, [this](const TuyaNewDatapoint &datapoint) {
      if (this->state_->current_values != this->state_->remote_values) {
        ESP_LOGD(TAG, "Light is transitioning, datapoint change ignored");
        return;
      }
  ...
  if (color_id_.has_value()) {
    this->parent_->register_listener(*this->color_id_, [this](const TuyaNewDatapoint &datapoint) {
      if (this->state_->current_values != this->state_->remote_values) {
        ESP_LOGD(TAG, "Light is transitioning, datapoint change ignored");
        return;
      }
      ...
      switch (*this->color_type_) {
        case TuyaNewColorType::RGBHSV:
        case TuyaNewColorType::RGB: {
...
void TuyaNewLight::dump_config() {
  ESP_LOGCONFIG(TAG, "TuyaNew Dimmer:");
  if (this->dimmer_id_.has_value()) {
...
light::LightTraits TuyaNewLight::get_traits() {
...
void TuyaNewLight::setup_state(light::LightState *state) { state_ = state; }

void TuyaNewLight::write_state(light::LightState *state) {
