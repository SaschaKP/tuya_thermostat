...
#include "tuyanew_select.h"

namespace esphome {
namespace tuyanew {

static const char *const TAG = "tuyanew.select";

void TuyaNewSelect::setup() {
  this->parent_->register_listener(this->select_id_, [this](const TuyaNewDatapoint &datapoint) {
...
void TuyaNewSelect::control(size_t index) {
  if (this->optimistic_)
...
void TuyaNewSelect::dump_config() {
  LOG_SELECT("", "TuyaNew Select", this);
  ESP_LOGCONFIG(TAG,
