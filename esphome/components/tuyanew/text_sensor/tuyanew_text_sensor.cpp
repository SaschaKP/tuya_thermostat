...
static const char *const TAG = "tuyanew.text_sensor";

void TuyaNewTextSensor::setup() {
  this->parent_->register_listener(this->sensor_id_, [this](const TuyaNewDatapoint &datapoint) {
    switch (datapoint.type) {
      ...
      default:
        ESP_LOGW(TAG, "Unsupported data type for tuyanew text sensor %u: %#02hhX", datapoint.id, (uint8_t) datapoint.type);
        break;
    }
  });
}

void TuyaNewTextSensor::dump_config() {
  ESP_LOGCONFIG(TAG,
                "TuyaNew Text Sensor:\n"
